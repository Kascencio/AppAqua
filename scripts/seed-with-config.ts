...
const DB_CONFIG = {
  ...
  port: 3306,
  user: 'root',
  password: 'Mvergel*', // Cambiar por tu contraseña
  database: 'u889902058_sonda0109' // Cambiar por el nombre de tu base de datos
}
...
async function seedAuthData() {
  ...
  try {
    ...
    if (error.code === 'ECONNREFUSED') {
      console.log('\n💡 Sugerencias para resolver el error de conexión:')
      console.log('1. Verifica que MySQL esté ejecutándose')
      console.log('2. Ajusta la configuración de DB_CONFIG en este archivo')
      console.log('3. Crea la base de datos "aquamonitor" si no existe')
      console.log('4. Verifica las credenciales de MySQL')
    }
    throw error
  } finally {
    if (db) {
