    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (parsed.type === "alerta.deleted") {
          const deletedId = Number(parsed.data?.id_alertas)
          if (!deletedId) return
          setLocalAlerts((prev) => prev.filter((item) => item.id_alertas !== deletedId))
          return
        }

        if (parsed.type !== "alerta.created" && parsed.type !== "alerta.updated") {
          return
        }

        const incoming = parsed.data
        if (!incoming?.id_alertas) return

        setLocalAlerts((prev) => {
          const exists = prev.some((item) => item.id_alertas === incoming.id_alertas)
          if (exists) {
            return prev.map((item) => (item.id_alertas === incoming.id_alertas ? { ...item, ...incoming } : item))
          }
          return [{ ...incoming, read: false }, ...prev]
        })

        if (parsed.type === "alerta.created") {
          toast({
            title: "Nueva alerta en tiempo real",
            description: incoming.descripcion || "Se recibió una nueva alerta del sistema.",
            duration: 4000,
          })
        }
      } catch {}
