...
import type { Organizacion } from "./columns"
...
export function EditOrganizacionDialog({
  open,
  onOpenChange,
  organizacion,
  onSuccess,
}: EditOrganizacionDialogProps) {
  ...
  // Load organizacion data when dialog opens
  useEffect(() => {
    if (open && organizacion) {
      form.reset({
        ...
        direccion: "", // Not in current interface, would need to be added
        estado: organizacion.estado as "activa" | "inactiva",
        descripcion: "", // Not in current interface, would need to be added
      })
    }
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      const response = await fetch(`/api/organizaciones/${organizacion.id_organizacion}`, {
        method: "PATCH",
        headers: {
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Modifica los datos de la organización. Los campos marcados con * son obligatorios.
          </DialogDescription>
        </DialogHeader>
